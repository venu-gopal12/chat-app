...
const RightSidebar = ({ selectedUser }) => {
  return selectedUser && (
    ...
    ${selectedUser ? 'max-md:hidden md:flex md:flex-col' : ''}`}>
  ...
  {imagesDummyData.map((url, index) => (
    <div key={index} onClick={() => window.open(url)} 
      ...
      <button className='absolute bottom-5 left-1/2
transform -translate-x-1/2 bg-gradient-to-r from-purple-400 to-violet-400
text-white border-none text-sm font-light py-2 px-20 
rounded-full cursor-pointer mt-4' >Logout</button>
    </div>
  )
}
