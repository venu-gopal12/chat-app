      ...
      <div className='w-5/6 max-w-2xl backdrop-blur-2xl text-gray-300 border-2 border-gray-600 flex items-center justify-between max-sm:flex-col-reverse rounded-lg'>
        <form onSubmit={handleSubmit} className="flex flex-col gap-5 p-10 flex-1" >
          ...
          <label htmlFor="avatar" className='flex items-center gap-3 cursor-pointer'>
            ...
            {/* Display the selected image preview, or a placeholder if none is selected */}
            <img
          ...
          className='p-2 border border-gray-500 rounded-md focus:outline-none focus:ring-2
          focus:ring-violet-500 resize-none h-24' rows={4}
          onChange={(e)=>setBio(e.target.value)} value={bio}
          ></textarea>
          <button type='submit'
          className='bg-violet-500 text-white py-2 rounded-md hover:bg-violet-600 transition'
          >Save</button>
          </form>
          <img className={`w-40 max-sm:w-32 m-10 p-5 ${selectedImg && 'rounded-full'}`}
